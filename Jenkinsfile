    ...
    stage('Deploy to App Service') {
        withCredentials([azureServicePrincipal('service-principal'), string(credentialsId: 'dockerPassword', variable: 'dockerPassword'), string(credentialsId: 'databasePassword', variable: 'dbPass')]) {
          ...
          sh """
              ...
              az deployment group create --resource-group \$resourceGroup  --name \$deploymentName --template-file azureDeploy.json --parameters dev.parameters.json --parameters dockerRegistryPassword=${dockerPassword} --parameters linuxFxVersion="DOCKER|gurpritsingh/bufalo-fullstack:${env.BUILD_NUMBER}" --parameters databasePassword=$dbPass --parameters dockerRegistryPassword=${dockerPassword} --parameters databaseUrl='postgres://postgres:$dbPass@188.166.87.169:5432/gobuff_realworld_example_app_production?sslmode=disable' --parameters testDatabaseUrl='postgres://postgres:$dbPass@188.166.87.169:5432/gobuff_realworld_example_app_production?sslmode=disable' --parameters databaseHost='188.166.87.169' --parameters waitDbHosts='188.166.87.169:5432'
            """
          }
      }
      stage('Post Cleanup'){
